...
def run(args, predict_dir, num_cls):
    ...
    for iter, pack in tqdm(enumerate(dataloader)):
        n_images += 1
        try:
            cam_dict = np.load(os.path.join("/data/nips/qqqqq/New_SFC9", predict_dir, pack['name'][0] + '.npy'), allow_pickle=True).item()
            keys = cam_dict['keys']

            # cams =( cam_dict['IS_CAM1']+cam_dict['IS_CAM'])
            ...
            # mask = np.array(Image.open(os.path.join(args.gt_path,  pack['name'][0].split("_")[2] + '.png')))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--gt_path", default='/data/tfl/VOCdevkit/VOC2012/SegmentationClass', type=str)
    ...
    if args.dataset == 'voc':
        ...
        args.gt_path = "/data/nips/COCO/coco_seg_anno"
