...
class Net(nn.Module):
    ...
    def get_seed(self, norm_cam, label, feature): # feature: 4, 2048, 32, 32
        n,c,h,w = norm_cam.shape # 4, 21, 32, 32])
        seeds = torch.zeros((n,h,w,c)).cuda()
        feature_s = feature.view(n,-1,h*w) # torch.Size([4, 2048, 1024])
        feature_s = feature_s/(torch.norm(feature_s,dim=1,keepdim=True)+1e-5)
        correlation = F.relu(torch.matmul(feature_s.transpose(2,1), feature_s),inplace=True).unsqueeze(1) # [4, 1, 1024, 1024]
        ...
        inter = (correlation * cam_flatten).sum(-1) #[4, 1, 1024, 1024] * [4, 21, 1, 1024] = ([4, 21, 1024])
        union = correlation.sum(-1) + cam_flatten.sum(-1) - inter # [4, 21, 1024]
        miou = (inter/union).view(n,self.num_cls,h,w) #[n,21,h,w]
        miou[:,0] = miou[:,0]*0.5 # torch.Size([4, 21, 32, 32])num_classes
        belonging = miou.argmax(1) # torch.Size([4, 32, 32])
        seeds = seeds.scatter_(-1, belonging.view(n,h,w,1), 1).permute(0,3,1,2).contiguous()
        seeds = seeds * label # 4, 21, 32, 32
        return seeds
    ...
    def prototype(self, norm_cam, feature, valid_mask):
        ...
        seeds = F.interpolate(norm_cam, feature.shape[2:], mode='nearest')
    ...
    def forward(self, x, valid_mask, my_label= None, epoch=None, index=None, train= None):
        ...
        # norm_cam = self.get_seed(norm_cam.clone(), valid_mask.clone(), sem_feature.clone())
...
class CAM(Net):
    ...
    def forward(self, x, label):
        ...
        # norm_cam = self.get_seed(norm_cam.clone(), label.unsqueeze(0).clone(), hie_fea.clone())
